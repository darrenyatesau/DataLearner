...
import weka.classifiers.trees.j48ts.BinC45ModelSelectionTS;
import weka.classifiers.trees.j48ts.C45ModelSelectionTS;
import weka.classifiers.trees.j48ts.C45PruneableClassifierTreeTS;
import weka.classifiers.trees.j48ts.ClassifierTreeTS;
import weka.classifiers.trees.j48ts.ModelSelectionTS;
import weka.classifiers.trees.j48ts.PruneableClassifierTreeTS;
...
public class J48TS
  ...
  public Capabilities getCapabilities() {
    ...
    try {
      if (!m_reducedErrorPruning)
        result = new C45PruneableClassifierTreeTS(null, !m_unpruned, m_CF, m_subtreeRaising, !m_noCleanup).getCapabilities();
      else
        result = new PruneableClassifierTreeTS(null, !m_unpruned, m_numFolds, !m_noCleanup, m_Seed).getCapabilities();
    }
    catch (Exception e) {
  ...
  public void buildClassifier(Instances instances) 
    ...
    ModelSelectionTS modSelection;

    if (m_binarySplits)
      modSelection = new BinC45ModelSelectionTS(m_minNumObj, instances);
    else
      modSelection = new C45ModelSelectionTS(m_minNumObj, instances);
    if (!m_reducedErrorPruning)
      m_root = new C45PruneableClassifierTreeTS(modSelection, !m_unpruned, m_CF,
					    m_subtreeRaising, !m_noCleanup);
    else
      m_root = new PruneableClassifierTreeTS(modSelection, !m_unpruned, m_numFolds,
					   !m_noCleanup, m_Seed);
    m_root.buildClassifier(instances);
    if (m_binarySplits) {
      ((BinC45ModelSelectionTS)modSelection).cleanup();
    } else {
