 ...
 */
 ...
 /*
 * SPAARC.java, Copyright (C) 2018 Darren Yates
 * Modified version of the SimpleCart algorithm, code copyright by their respective authors.
*/
 ...
 * <br>
 * Darren Yates, Md Zahidul Islam, Junbin Gao (2018). SPAARC: A Fast Decision Tree Algorithm. 
 * Presented at 2018 Australasian Data Mining (AusDM 2018) conference, Bathurst, NSW 28-30 November 2018.
 * To be published in 2019 in 'Communications in Computer and Information Sciences', Springer.
 * <br>
 <!-- globalinfo-end -->	
 *
 ...
 * BibTeX:
 * <pre>
 * &#64;article{yates2018spaarc,
 *   title={SPAARC: A Fast Decision Tree Algorithm},
 *   author={Darren Yates, Md Zahidul Islam and Junbin Gao},
 *   conference={Australasian Data Mining (AusDM 2018) conference},
 *   year={2018},
 * }
 * </pre>
 * 
 * @author (additional code) Darren Yates (dyates@csu.edu.au)
 * @version $Revision: 10490.1 $
...
public class SPAARC extends RandomizableClassifier implements
   ...
   //---------------------------------------------------------------------------------------
    ...
    public String globalInfo() {
        return "Class implementing SPAARC decision tree algorithm.\n"
    ...
    public TechnicalInformation getTechnicalInformation() {
        ...
        result.setValue(Field.AUTHOR, "Darren Yates, Md Zahidul Islam, Junbin Gao");
        result.setValue(Field.YEAR, "2018");
        result.setValue(Field.TITLE, "SPAARC: A Fast decision Tree");
        result.setValue(Field.ORGANIZATION, "Australasian Data Mining conference (AusDM 2018)");
        result.setValue(Field.LOCATION, "Bathurst, NSW, Australia");
        result.setValue(Field.BOOKTITLE, "Communications in Computer and Information Science");
    ...
    public void buildClassifier(Instances data) throws Exception {
        ...
        //---------------------------------------------------------------------------------------
        ArrayList<Integer> sortedAtts = new ArrayList<>();
        attEntropy = new double[data.numAttributes()];
        //---------------------------------------------------------------------------------------
        ...
        if (!m_Prune) {
            ...
            makeTree(data, data.numInstances(), sortedIndices, weights, classProbs,
                    totalWeight, m_minNumObj, m_Heuristic, treeDepth+1, sortedAtts);
            return;
        }
...
//            m_ClassValue = Utils.missingValue();
        ...
        int numberOfAttrs = data.numAttributes();
        int selAttrCount = numberOfAttrs/2;
        double[][][] dists = new double[numberOfAttrs][0][0];
        double[][] props = new double[numberOfAttrs][0];
        double[][] totalSubsetWeights = new double[numberOfAttrs][2];
...
//-------------------------------------------------------------------------------
            System.arraycopy(giniGains, 0, attEntropy, 0, numberOfAttrs);
            sortedAtts = new ArrayList<>(Arrays.asList(quickSort(attEntropy, numberOfAttrs)));
            sortedAtts.removeAll(Arrays.asList(data.classIndex()));
        } else {
            for (int i = 0; i < selAttrCount; i++) {
                Attribute att = data.attribute(sortedAtts.get(i));
                if (att.isNumeric()) {
                    // numeric attribute
                    splits[sortedAtts.get(i)] = numericDistribution(props, dists, att, sortedIndices[sortedAtts.get(i)],
                            weights[sortedAtts.get(i)], totalSubsetWeights, giniGains, data);
                } else {
                    // nominal attribute
                    splitString[sortedAtts.get(i)] = nominalDistribution(props, dists, att,
                            sortedIndices[sortedAtts.get(i)], weights[sortedAtts.get(i)], totalSubsetWeights, giniGains, data,
                            useHeuristic);
                }
            }
        }
...
//--------------------------------------------------------------------------------------    
    private Integer[] quickSort(double[] entropy, int numInstances) {
        final double[] attrData = new double[numInstances];
        Integer[] attrIndices = new Integer[numInstances];
        for (int i = 0; i < numInstances; i++) {
            attrData[i] = entropy[i];
            attrIndices[i] = i;
        }
        Arrays.sort(attrIndices, new Comparator<Integer>() {
            @Override 
            public int compare(final Integer o1, final Integer o2) {
                return Double.compare(attrData[o2], attrData[o1]);
            }
        });
        return attrIndices;
    }
//--------------------------------------------------------------------------------------    
    ...
    private void prune(double alpha) throws Exception {
        ...
        while (prune) {
            ...
            SPAARC nodeToPrune = nodeToPrune(nodeList);
 ...
 */        
//----------------------------------------------------------------------------
        double hopStart = data.instance(sortedIndices[0]).value(att);
        double valueRange = 0;
        if (missingStart > 0) valueRange = data.instance(sortedIndices[missingStart-1]).value(att) - hopStart;
        double hopStep = valueRange/numberOfHops;
...
//---------------------------------------------------------------------------------------
            if (inst.value(att) > hopPoint && inst.value(att) > currSplit) {
//---------------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------------
                    splitPoint = (inst.value(att) + data.instance(sortedIndices[i-1]).value(att)) / 2.0;
//---------------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------------
     ...
     * successor nodes.
     *
     * @param subsetIndices sorted indecis of instances for each attribute for
     * two successor node
    ...
    private SPAARC nodeToPrune(Vector<SPAARC> nodeList) {
        ...
        SPAARC returnNode = nodeList.elementAt(0);
        double baseAlpha = returnNode.m_Alpha;
        for (int i = 1; i < nodeList.size(); i++) {
            SPAARC node = nodeList.elementAt(i);
    ...
    public String toString() {
        if ((m_ClassProbs == null) && (m_Successors == null)) {
            return "SPAARC Tree: No model built yet.";
        }

        return "SPAARC Decision Tree\n" + toString(0) + "\n\n"
