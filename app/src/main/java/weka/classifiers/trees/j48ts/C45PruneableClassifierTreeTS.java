...
public class C45PruneableClassifierTreeTS
	 ...
	 */
	public C45PruneableClassifierTreeTS(ModelSelectionTS toSelectLocModel,
	...
	private void collapse() {
		...
		if (!m_isLeaf) {
			...
			if (errorsOfSubtree >= errorsOfTree - 1E-3) {
				...
				m_localModel = new NoSplitTS(localModel().distribution());
			} else
	...
	private void prune() throws Exception {
		...
		C45PruneableClassifierTreeTS largestBranch;
		int i;
...
//---------------------------------------------------------------------------------------------------
	...
	private double getEstimatedErrorsForBranch(Instances data)
		...
		if (m_isLeaf)
			return getEstimatedErrorsForDistribution(new DistributionTS(data));
		else {
	 ...
	 * @return the estimated errors
	 */
	private double getEstimatedErrorsForDistribution(DistributionTS
															 theDistribution) {
