...
public class BFTreeTS
	...
	public void buildClassifier(Instances data) throws Exception {
		...
		BFTreeTS[] m_roots = new BFTreeTS[m_numFoldsPruning];
...
//---------------------------------------------------------------------------------------------------
				...
				m_roots[i].m_isLeaf = true;
				EvaluationTS eval = new EvaluationTS(test[i]);
				eval.evaluateModel(m_roots[i], test[i]);
				double error;
	...
	protected void makeTree(FastVector BestFirstElements, Instances data,
		...
		if (totalWeight < 2 * minNumObj || branchProps[0] == 0
			...
			if (BestFirstElements.size() != 0) {
				...
				BFTreeTS nextSplitNode = (BFTreeTS) nextSplitElement.elementAt(0);
//---------------------------------------------------------------------------------------------------
//				if (MainActivity.killThread == true) {
//---------------------------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------------------------
		...
		if (gain == 0 || preExpansion == m_Expansion) {
			for (int i = 0; i < BestFirstElements.size(); i++) {
				...
				BFTreeTS node = (BFTreeTS) element.elementAt(0);
				node.makeLeaf(data);
			}
...
//---------------------------------------------------------------------------------------------------
	...
	protected boolean makeTree(FastVector BestFirstElements, BFTreeTS root,
		...
		// node to split
		BFTreeTS nodeToSplit = (BFTreeTS) firstElement.elementAt(0);
	...
	protected void makeTree(FastVector BestFirstElements, BFTreeTS root,
		...
		if (totalWeight < 2 * minNumObj || branchProps[0] == 0
			...
			BFTreeTS nextSplitNode = (BFTreeTS)
					((FastVector) BestFirstElements.elementAt(0)).elementAt(0);
//---------------------------------------------------------------------------------------------------
			if (MainActivity.killThread == false) {
//---------------------------------------------------------------------------------------------------
		...
		// gini gain or information gain is not 0
			...
			// split the node and cauculate error rate of this temporary tree
				...
				for (int i = 0; i < 2; i++) {
					...
					EvaluationTS eval = new EvaluationTS(test);
//---------------------------------------------------------------------------------------------------
				if (MainActivity.killThread == false) {
//---------------------------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------------------------
	...
	public static void main(String[] args) {
		runClassifier(new BFTreeTS(), args);
	}
}
