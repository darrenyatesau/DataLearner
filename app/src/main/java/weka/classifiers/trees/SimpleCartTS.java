...
public class SimpleCartTS
	 ...
	 * Successor nodes.
	 */
	private SimpleCartTS[] m_Successors;
...
//---------------------------------------------------------------------------------------------------
	 ...
	 * @throws Exception if something goes wrong
	 */
	private void makeTree(Instances data, int totalInstances, int[][] sortedIndices,
		...
		if (totalWeight < 2 * minNumObj || giniGains[attIndex] == 0 ||
			...
			m_isLeaf = false;
			m_Successors = new SimpleCartTS[2];
			for (int i = 0; i < 2; i++) {
//---------------------------------------------------------------------------------------------------
				if (MainActivity.killThread == true) break;
//---------------------------------------------------------------------------------------------------
				m_Successors[i] = new SimpleCartTS();
...
//	  splitPoint = Math.rint((inst.value(att) + currSplit)/2.0*100000)/100000.0;
	 ...
	 */
	private void fillInnerNodes(Vector nodeList) {
		if (!m_isLeaf) {
			nodeList.add(this);
			for (int i = 0; i < m_Successors.length; i++)
	...
	public static void main(String[] args) {
		runClassifier(new SimpleCartTS(), args);
	}
}
