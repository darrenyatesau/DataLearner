...
import weka.classifiers.trees.adtreets.ReferenceInstancesTS;
import weka.classifiers.trees.adtreets.SplitterTS;
import weka.classifiers.trees.adtreets.TwoWayNominalSplitTS;
import weka.classifiers.trees.adtreets.TwoWayNumericSplitTS;
...
public class ADTreeTS
  ...
  public void initClassifier(Instances instances) throws Exception {
    ...
    m_posTrainInstances = new ReferenceInstancesTS(m_trainInstances,
						 m_trainInstances.numInstances());
    m_negTrainInstances = new ReferenceInstancesTS(m_trainInstances,
						 m_trainInstances.numInstances());
    ...
    double rootPredictionValue = calcPredictionValue(m_posTrainInstances,
						     m_negTrainInstances);
    m_root = new PredictionNodeTS(rootPredictionValue);
  ...
  private void boost() throws Exception {
	...
	m_search_bestSplitter.instancesDownBranch(i, m_search_bestPathNegInstances);
      ...
      PredictionNodeTS newPredictor = new PredictionNodeTS(predictionValue);
   ...
   * @exception Exception if search fails
   */
  private void searchForBestTestSingle(PredictionNodeTS currentNode,
   ...
   * @exception Exception if search fails
   */
  private void goDownAllPathsSingle(PredictionNodeTS currentNode,
				    Instances posInstances, Instances negInstances)
    throws Exception {
   ...
   * @exception Exception if search fails
   */
  private void goDownHeaviestPathSingle(PredictionNodeTS currentNode,
					Instances posInstances, Instances negInstances)
    throws Exception {

    SplitterTS heaviestSplit = null;
   ...
   * @exception Exception if search fails
   */
  private void goDownZpurePathSingle(PredictionNodeTS currentNode,
				     Instances posInstances, Instances negInstances)
    throws Exception {

    double lowestZpure = m_search_smallestZ; // do z-pure cutoff
    PredictionNodeTS bestPath = null;
   ...
   * @exception Exception if search fails
   */
  private void goDownRandomPathSingle(PredictionNodeTS currentNode,
				      Instances posInstances, Instances negInstances)
    throws Exception {

    FastVector children = currentNode.getChildren();
    SplitterTS split = (SplitterTS) children.elementAt(getRandom(children.size()));
   ...
   * @param negInstances the negative-class instances that apply at this node
   */
  private void evaluateNominalSplitSingle(int attIndex, PredictionNodeTS currentNode,
					  Instances posInstances, Instances negInstances)
  {
    ...
    if (indexAndZ[1] < m_search_smallestZ) {
      ...
      m_search_bestSplitter = new TwoWayNominalSplitTS(attIndex, (int) indexAndZ[0]);
   ...
   */
  private void evaluateNumericSplitSingle(int attIndex, PredictionNodeTS currentNode,
					  Instances posInstances, Instances negInstances,
    ...
    if (splitAndZ[1] < m_search_smallestZ) {
      ...
      m_search_bestInsertionNode = currentNode;
      m_search_bestSplitter = new TwoWayNumericSplitTS(attIndex, splitAndZ[0]);
      m_search_bestPathPosInstances = posInstances;
      m_search_bestPathNegInstances = negInstances;
    }
  }
  ...
  private String toString(PredictionNodeTS currentNode, int level) {
	...
	PredictionNodeTS child = split.getChildForBranch(j);
	if (child != null) {
	  text.append("\n");
  ...
  private void graphTraverse(PredictionNodeTS currentNode, StringBuffer text,
    ...
    for (Enumeration e = currentNode.children(); e.hasMoreElements(); ) {
      ...
      for (int i=0; i<split.getNumOfBranches(); i++) {
	PredictionNodeTS child = split.getChildForBranch(i);
	if (child != null) {
  ...
  public Object clone() {
    ...
    ADTreeTS clone = new ADTreeTS();

    if (m_root != null) { // check for initialization first
      clone.m_root = (PredictionNodeTS) m_root.clone(); // deep copy the tree
	...
	clone.m_posTrainInstances =
	  new ReferenceInstancesTS(m_trainInstances, m_posTrainInstances.numInstances());
	clone.m_negTrainInstances =
	  new ReferenceInstancesTS(m_trainInstances, m_negTrainInstances.numInstances());
   ...
   */
  public void merge(ADTreeTS mergeWith) throws Exception {
  ...
  public static void main(String [] argv) {
    runClassifier(new ADTreeTS(), argv);
  }
}
