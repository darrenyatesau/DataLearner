	...
	public static VaryNodeTS makeVaryNode(int iNode, FastVector nRecords, Instances instances) {
		...
		for (int iChild = 1; iChild < nValues; iChild++) {
			if (nChildRecords[iChild].size() > nCount) {
                ...
                _VaryNode.m_ADNodes = new ADNodeTS[nValues];
		for (int iChild = 0; iChild < nValues; iChild++) {
	...
	public static ADNodeTS makeADTree(int iNode, FastVector nRecords, Instances instances) {
		ADNodeTS _ADNode = new ADNodeTS();
                ...
                if (nRecords.size() < MIN_RECORD_SIZE) {
                  ...
                  _ADNode.m_VaryNodes = new VaryNodeTS[instances.numAttributes() - iNode];
...
//System.out.println();
        ...
        public static void main(String [] argv) {
            try {
                ...
                ADNodeTS ADTree = ADNodeTS.makeADTree(instances);
