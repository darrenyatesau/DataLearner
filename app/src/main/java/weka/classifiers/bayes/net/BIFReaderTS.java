...
public class BIFReaderTS
	 ...
	 */
	public BIFReaderTS processFile(String sFile) throws Exception {
		m_sFile = sFile;
	...
	public BIFReaderTS processString(String sStr) throws Exception {
        DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
        factory.setValidating(true);
...
//	} // selectNodeList
   ...
   FastVector selectProperty(Node item) throws Exception {
	 ...
	 */
	public int missingArcs(BayesNetTS other) {
		try {
			Sync(other);
			int nMissing = 0;
	 ...
	 * @return nr of missing arcs
	 */
	public int extraArcs(BayesNetTS other) {
		try {
			Sync(other);
			int nExtra = 0;
			for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++) {
	 ...
	 */
	public double divergence(BayesNetTS other) {
		try {
			Sync(other);
			// D: divergence
			double D = 0.0;
	 ...
	 * @return nr of missing arcs
	 */
	public int reversedArcs(BayesNetTS other) {
		try {
			Sync(other);
			int nReversed = 0;
		    for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++) {
    ...
    public static void main(String[] args) {
        try {
            BIFReaderTS br = new BIFReaderTS();
